...
local function deceased()
    ...
    TaskStartScenarioInPlace(PlayerPedId(), joaat("WORLD_HUMAN_GRAVE_MOURNING_KNEEL"), 20000, true, false, false, false)

    -- Notify the player
    RSGCore.Functions.Notify('The pigs are coming...', 'primary')

    -- Spawn pigs
    local pigHash = joaat("A_C_Pig_01")
    ...
    for _, pig in ipairs(pigs) do
        TaskStartScenarioInPlace(pig, joaat("WORLD_ANIMAL_PIG_EAT"), 20000, true, false, false, false)
    end
...
-- Not used in this script, but you can use this to remove the blips if needed
local function RemoveGraveBlips()
...
local function CreateGraveBlip(x, y, z)
...
CreateThread(function()
    while true do
        ...
        if #graveBlips == 0 then
            for _, v in pairs(Config.grave) do
